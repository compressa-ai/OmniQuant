...
def omniquant(
    ...
    for i in range(len(layers)):
        ...
        if args.epochs > 0:
            ...
            num_linears = len([m for m in qlayer.modules() if isinstance(m, QuantLinear)])
            m_index = -1
            mean_results = []
            ...
            for m in qlayer.modules():
                ...
                for epochs in range(args.epochs):
                    ...
                    def layer_err(p, m_index, index):
                        with torch.cuda.amp.autocast():
                            qlayer.smooth_and_quant_temporary(m_index, p)
                            quant_out = qlayer(
                    ...
                    for j in range(args.nsamples//args.batch_size):
                        ...
                        res = opt.minimize(lambda p: layer_err(p, m_index, index), init, method='Nelder-Mead')
                    ...
                    logger.info(f"layer {i} linear {m_index} iter {epochs} loss:{loss_mean} norm:UNK params:{mean_res} max memory_allocated {torch.cuda.max_memory_allocated(lm._device) / 1024**2} ")

            qlayer.clear_temp_variable()
            del optimizer

            assert m_index == num_linears - 1
        ...
        for m in qlayer.modules():
            ...
            qlayer.smooth_and_quant_inplace(m_index, mean_results[m_index])

        # TODO: will break of epochs == 0
        assert m_index == num_linears - 1
