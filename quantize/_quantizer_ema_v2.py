...
class UniformAffineQuantizer(nn.Module):
    def __init__(
        ...
        self.xmin = nn.Parameter(
            torch.Tensor([0]).cuda(), requires_grad=False
        )
        self.xmax = nn.Parameter(
            torch.Tensor([0]).cuda(), requires_grad=False
        )
    ...
    def per_token_dynamic_calibration(self, x):
        ...
        if self.lwc:  # Weight
            ...
            with torch.no_grad():
                self.xmin.data = self.xmin + torch.mean(self.xmins)
                self.xmax.data = self.xmax + torch.mean(self.xmaxs)

            xmin = self.xmin
            xmax = self.xmax

            self.xmins[int(self.num_iters) % self.history_length] = float(x.min())
            self.xmaxs[int(self.num_iters) % self.history_length] = float(x.max())

            self.num_iters += 1
        elif self.training:
            if self.num_iters % 100 == 0:
                print(f'!!! I am alive and I update stats')

            with torch.no_grad():
                self.xmin.data = (1 - self.alpha) * self.xmin + self.alpha * torch.mean(self.xmins)
                self.xmax.data = (1 - self.alpha) * self.xmax + self.alpha * torch.mean(self.xmaxs)
