...
def kinda_smooth_ln_fcs_temporary(ln, fcs, scales, shifts):
    # ln.use_temporary_parameter = True
    if not isinstance(fcs, list):
        fcs = [fcs]
    # if hasattr(ln, 'bias') and ln.bias is not None:
...
def kinda_smooth_fc_fc_temporary(fc1, fc2, scales, shifts=None):
    # only support for v_proj and out_proh now.
    # fc1.use_temporary_parameter = True
    fc2.use_temporary_parameter = True
    # if hasattr(fc1, 'temp_weight'):
...
def kinda_smooth_q_k_temporary(q_proj, k_proj, scales):
    # q_proj.use_temporary_parameter = True
    k_proj.use_temporary_parameter = True
    # q_proj.temp_weight = q_proj.temp_weight/scales.view(-1,1)
    # q_proj.temp_bias = q_proj.temp_bias/scales.view(-1)
...
def kinda_smooth_ln_fcs_inplace(ln, fcs, scales, shifts):
    # ln.use_temporary_parameter = False
    if not isinstance(fcs, list):
        fcs = [fcs]
    # if hasattr(ln, 'bias') and ln.bias is not None:
...
def kinda_smooth_fc_fc_inplace(fc1, fc2, scales, shifts=None):
    # only support for v_proj and out_proh now.
    # fc1.use_temporary_parameter = False
    fc2.use_temporary_parameter = False
    # fc1.bias.sub_(shifts)
...
def kinda_smooth_q_k_inplace(q_proj, k_proj, scales, ):
    # q_proj.use_temporary_parameter = False
    k_proj.use_temporary_parameter = False
    # q_proj.weight.div_(scales.view(-1,1))
    # q_proj.bias.div_(scales.view(-1))
...
def kinda_unsmooth_ln_fcs_temporary(ln, fcs, scales, shifts):
    # ln.use_temporary_parameter = True
    if not isinstance(fcs, list):
        fcs = [fcs]
    # if hasattr(ln, 'bias') and ln.bias is not None:
...
def kinda_unsmooth_fc_fc_temporary(fc1, fc2, scales, shifts=None):
    # only support for v_proj and out_proh now.
    # fc1.use_temporary_parameter = True
    # if hasattr(fc1, 'temp_weight'):
    #     fc1.temp_bias = fc1.temp_bias - shifts
...
def kinda_unsmooth_q_k_temporary(q_proj, k_proj, scales):
    # q_proj.use_temporary_parameter = True
    # q_proj.temp_weight = q_proj.temp_weight/scales.view(-1,1)
    # q_proj.temp_bias = q_proj.temp_bias/scales.view(-1)
    k_proj.temp_weight = k_proj.temp_weight / scales.view(-1, 1)
...
def kinda_unsmooth_ln_fcs_inplace(ln, fcs, scales, shifts):
    # ln.use_temporary_parameter = False
    if not isinstance(fcs, list):
        fcs = [fcs]
    # if hasattr(ln, 'bias') and ln.bias is not None:
    ...
    # ln.weight.div_(scales)
    for fc in fcs:
        # fc.bias.add_(-1 * fc.weight @ shifts)
        fc.weight.div_(scales.view(1, -1))


def kinda_unsmooth_fc_fc_inplace(fc1, fc2, scales, shifts=None):
    # only support for v_proj and out_proh now.
    # fc1.use_temporary_parameter = False
    # fc1.bias.sub_(shifts)
    # fc1.bias.div_(scales.view(-1))
    ...
    # fc2.bias.add_(-1 * fc2.weight @ shifts)
    fc2.weight.div_(scales.view(1, -1))


def kinda_unsmooth_q_k_inplace(q_proj, k_proj, scales, ):
    # q_proj.use_temporary_parameter = False
    # q_proj.weight.div_(scales.view(-1,1))
    # q_proj.bias.div_(scales.view(-1))
    k_proj.weight.div_(scales.view(-1, 1))
    # k_proj.bias.div_(scales.view(-1))
